...
# Bu, LangGraph in persistent memory (kalıcı hafıza) sistemi için gerekli sınıf.
# LangGraph te “hafıza” doğrudan LLM tarafından değil,
# arka planda bir checkpointer (kontrol noktası kaydedici) tarafından yönetiliyor.
# Ve SqliteSaver bunun SQLite tabanlı versiyonudur.
# yani: Her konuşmadan sonra State (mesaj geçmişi) bir SQLite dosyasına (checkpoint.db) kaydedilir.
# Bu sayede programı kapatıp tekrar açsan bile aynı thread_id yi kullanarak
# önceki konuşmaya geri dönebilirsin.

# print("03_add_memory: LangGraph'e persistent memory eklemek\n",
#       "Artik chatbot sadece bir kerelik cevap vermiyor;\n",
#       "ayni thread_id ile konuşmaya devam edersen önceki konuşmalari hatirliyor.\n",
#       "="*80+"\n")
...
# “State adlı bir sözlük olacak, içinde "messages" adında bir key bulunacak.
# Value'su bir "list" olacak,
# ve bu list "add_messages" adlı ek bir açıklama ile etiketlenmiş.”
...
def build_chatbot_with_memory_graph():
    ...
    """
    Burada çok önemli üç şey oluyor:

    1. sqlite3.connect("checkpoint.db")
        → “checkpoint.db” adinda bir veritabani dosyasi oluşturur.
        (Bu dosya LangGraph in hafiza deposudur.)

    2. SqliteSaver(...)
        → LangGraph in SqliteSaver sinifi, bu bağlantiyi alir
        ve state kayit / geri yükleme (save / restore) işlemlerini yönetir.

    3. check_same_thread=False
        → SQLite bağlantisi, ayni anda birden fazla thread tarafindan kullanilabilsin diye.
    """
    # Compile the graph WITH the checkpointer
    return graph_builder.compile(checkpointer=checkpointer)

# Bu fonksiyon bir “konuşma turu” (single turn) çalıştırır.
# Ama fark şu:
# Aynı thread_id’yi kullanırsan, bu tur önceki turlarla bağlantılı hale gelir.
# Yani:
# Yeni kullanıcı mesajı gelir (message)
# Daha önceki konuşma geçmişi SQLite’tan (checkpoint.db) yüklenir
# LLM, bu geçmişle birlikte yanıt üretir
# Güncel state tekrar diske kaydedilir
# 💡 Böylece bot “geçmişi hatırlayan” hale gelir.
